...
Monitors Hetzner's server auction in real time, scoring listings with a cluster-aware weighted formula (PassMark CPU benchmarks + RAM/drives/price/datacenter + ECC/NVMe). Produces deal badges, time-on-market urgency signals, and cluster-expansion simulations. Drives all MADFAM hardware procurement decisions — this is how the estate's dedicated control-plane and worker nodes and the CI builder were sourced (node identity lives only in the private operations record). CLI binary is `foundry-scout`; web dashboard at `sniper.madfam.io` is `deal-sniper`.
...
# Logs
enclii logs deal-sniper --env prod -f                    # live tail
enclii logs deal-sniper --env prod --since 1h -n 200     # last hour
...
# Rollback
enclii rollback deal-sniper --env prod                   # previous release
enclii rollback deal-sniper v42 --env prod

# Releases + deployment history
enclii releases deal-sniper -n 20
...
enclii domains list --service deal-sniper
enclii domains add my.example.com --service deal-sniper   # auto-provisions tunnel route + DNS
